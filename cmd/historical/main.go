...
func work(app *config.Application) {
	...
	for latest.Next(false) {
		...
		if err := latest.Error(); err != nil {
			...
			app.Logger.Println("Error: unable to retrieve user feed")
			break
		}
	}
	wg.Wait()
